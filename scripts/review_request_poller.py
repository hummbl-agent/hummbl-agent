...
Identity:
    The proposed bus identity is "review-poller". This identity is NOT yet
    registered in the bus writer's allowlist. Until the operator approves it,
    the poller operates in dry-run (read-only) mode by default. Use --post
    to attempt bus posts (will fail until the identity is registered).
...
# Proposed bus identity — NOT yet registered. Bus posts will fail until approved.
PROPOSED_BUS_IDENTITY = "review-poller"
...
def post_alert(alerts, identity=PROPOSED_BUS_IDENTITY):
    """Post a STATUS alert to the bus via the bus-global wrapper.

    Returns True on success, False on failure.
    Note: The 'review-poller' identity is NOT yet registered in the bus
    writer's allowlist. Posts will fail until the operator approves it.
    """
    try:
        import subprocess
...
def main():
    ...
    parser.add_argument(
        "--post",
        action="store_true",
        help="Attempt to post alerts to the bus. Default is dry-run (read-only). "
             "The proposed 'review-poller' identity is not yet registered; posts "
             "will fail until the operator approves it.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--identity",
        type=str,
        default=PROPOSED_BUS_IDENTITY,
        help=f"Bus identity to use when posting (default: {PROPOSED_BUS_IDENTITY}). "
             "Not yet registered in the bus allowlist.",
    )
