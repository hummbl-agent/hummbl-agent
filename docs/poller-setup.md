...
5. Posts a `STATUS` alert to the bus **only** when `--post` is passed AND
   the `hummbl-agent` identity is registered in the bus allowlist
...
2. The `hummbl-agent` identity is registered in the bus writer's allowlist.
...
The poller posts alerts using the bus identity `hummbl-agent`, which was
registered 2026-07-23 in the founder-mode canonical source (agent_identity.py
CANONICAL_AGENTS, bus_writer_core.py _RESERVED_AGENT_IDS, anvil-bus.py
PERMITTED_SENDERS, agent-roster.md). Trust=MEDIUM, Status=ACTIVE.

The poller defaults to dry-run (read-only) mode. Use `--post` to emit STATUS
alerts to the bus.
...
> **NOTE**: A task named `hummbl-hummbl-agent` was created and immediately
...
```powershell
Enable-ScheduledTask -TaskName "hummbl-hummbl-agent"
```
...
<dict>
    <key>Label</key>
    <string>com.hummbl.hummbl-agent</string>
    ...
    <string>/tmp/hummbl-hummbl-agent.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/hummbl-hummbl-agent.err</string>
    <key>RunAtLoad</key>
    <true/>
</dict>
</plist>
...
```cron
*/5 * * * * /usr/bin/python3 /path/to/hummbl-agent-reviewer/scripts/review_request_poller.py --quiet >> /var/log/hummbl-hummbl-agent.log 2>&1
```
...
| `--post` | Attempt to post alerts to the bus (fails until `hummbl-agent` identity is registered) |
...
| `--identity <name>` | Override bus identity (default: `hummbl-agent`) |
